...
import { getProducts, getMans, getMansModels } from "../services";
import { cdn, man_search, day_different, gadamrtveli, ruli, union, spidometri, heart, redact, manqana, flag_geo, done } from '../helpers';
import flag_usa from '../photos/usa.png'
import { Row, Container, Col, Form, Tabs, Tab, Dropdown } from "react-bootstrap";
...
const Main = () => {
  ...
  const [mansType, setMansType] = useState({
    TypeID: 0
  });
  const [manId, setManId] = useState(1);
  ...
  useEffect(() => {
    setFilters(prevState => ({ ...prevState, TypeID: mansType.TypeID }));
    getMans(mansType).then((result) => setMans(result));
  }, [mansType]);

  useEffect(() => {
    getProducts(filters).then((result) => setProducts(result));
  }, [filters]);

  useEffect(() => {
    let price_to = currency.current == 'GEL' ? prices.PriceTo : prices.PriceTo * currency.course['GEL'].price;
    let price_from = currency.current == 'GEL' ? prices.PriceFrom : prices.PriceFrom * currency.course['GEL'].price;
    if (price_to == '0') {
      price_to = ''
    }
    if (price_from == '0') {
    ...
    setFilters(prevState => ({ ...prevState, PriceFrom: price_from, PriceTo: price_to }))
  }, [prices]);

  useEffect(() => {
    getMansModels({ man_id: manId }).then((result) => setManModels(result));
  }, [manId]);
  ...
  return (
    <>
      <Container className="margin">
        <Breadcrumb />
        <Row>
          <Col className={`side-bar-underline side-bar-hide ${filterMenuVisible && 'side-bar-hide-active'}`} lg='4'>

            <div className="side-bar">
              <div className="sidebar-header d-none-lg ">
                <button onClick={() => setFilterMenuVisible(!filterMenuVisible)}>
                  {'<'}
                </button>
                <p>
                  ფილტრი
              ...
              <Tabs
                ...
                className="mb-3 justfy-center"
                onSelect={(el) => setMansType({ TypeID: el })}
              >
                <Tab
                  eventKey="0"
                  title={(
                    <svg width="30" height="15" viewBox="0 0 30 15" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                ...
                <Tab
                  eventKey="1"
                  title={(<svg width="22" height="18" viewBox="0 0 22 18" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="side-bar-padding side-bar-underline ">
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Select className="form-border" onChange={(el) => {
                    setFilters(prevState => ({ ...prevState, Mans: el.target?.value }));
                    setManId(el.target?.value);
                  }}>
                    <option value=''>ყველა</option>
                    {mans?.map((item, key) => (
                      <option value={item.man_id}>{item.man_name}</option>
                    ))}
                  </Form.Select>
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>კატეგორია</Form.Label>
                  <Form.Select className="form-border" onChange={(el) => setFilters(prevState => ({ ...prevState, Mans: `${manId}.${el.target?.value}` }))}>
                    <option value=''>ყველა</option>
                    {manModels?.data?.map((item, key) => (
              ...
              <div className="side-bar-padding">
                <div className="flex flex-betweenn ">
                  <Form.Label htmlFor="price">ფასი</Form.Label>
                ...
                <div className="flex">
                  <Form.Control
                    // value={currency.current == 'GEL' ? filters.PriceFrom : filters.PriceFrom * currency.course[currency.current].price}
                    type="number"
                    placeholder="დან"
                    onChange={(el) => setPrices(prevState => ({ ...prevState, PriceFrom: el.target?.value }))}
                  />
                  <div>
                    <h3 className="gray">-</h3>
                  </div>
                  <Form.Control
                    // value={currency.current == 'GEL' ? filters.PriceTo : filters.PriceTo * currency.course[currency.current].price}
                    type="number"
                    placeholder="მდე"
                    onChange={(el) => setPrices(prevState => ({ ...prevState, PriceTo: el.target?.value }))}
                  />
                </div>
              </div>
              <div className="side-bar-btn-shadow side-bar-padding margin">
                <buttom className="btn-find">ძებნა {products?.data?.meta?.total}</buttom>
              </div>
            </div>
          </Col>
          <Col >
            <Col className="filter-cont" >
              <buttom className="filter-button" onClick={() => {
                setFilterMenuVisible(!filterMenuVisible)
              }}>ფილტრი</buttom>
            </Col>
            <Row>
              <Col >
                <p className="gancxadeba">{products?.data?.meta?.total} განცხადება</p>
              </Col>
                  ...
                  <Col >
                    <div className="flex-end">
                      <Dropdown>
                        ...
                        <Dropdown.Menu>
                          <Dropdown.Item href="#/action-1">ბოლო 1 საათი</Dropdown.Item>
                          <Dropdown.Item href="#/action-2">ბოლო 2 საათი</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">ბოლო 3 საათი</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">ბოლო 1 დღის</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">ბოლო 2 დღის</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">ბოლო 3 დღის</Dropdown.Item>
                        </Dropdown.Menu>
                      </Dropdown>
                      <Dropdown>
                        <Dropdown.Toggle variant="success" id="dropdown-basic">
                        ...
                        <Dropdown.Menu>
                          <Dropdown.Item href="#/action-1">თარიღი კლებადი</Dropdown.Item>
                          <Dropdown.Item href="#/action-2">თარიღი ზრდადი</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">ფასი კლებადი</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">ფასი ზრდადი</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">გარბენი კლებადი</Dropdown.Item>
                          <Dropdown.Item href="#/action-3">გარბენი ზრდადი</Dropdown.Item>
                        </Dropdown.Menu>
                      </Dropdown>
                    </div>
                  </Col>
            ...
            {products?.data?.items?.map((item, key) => (
              <div key={key} className="card">
                <Container className="card-container-padd">
                  <Row className="justify-content-md-between">
                    <Col className="d-none-lg mb-4">
                      <p className="title title-media">{man_search(item.man_id, mans)?.man_name} <span className="title_span"> {item.car_model}</span></p>
                      <div className="d-flex jc-between ai-center">
                        <h1 className="align-text align-text-start price price_media m-0">{Math.round(item.price_usd * currency.course[currency.current].price).toLocaleString('en-US', { maximumFractionDigits: 2 })}</h1>
                        <p className="align-text m-0">{item.customs_passed ? <span className="green-color ">{done} განბაჟებული {flag_geo}</span> : <span className="orange-color">განუბაჟებელი {<img src={flag_usa} alt="flags" />}</span>}</p>
                      </div>
                    </Col>
                    <Col lg='4'  >
                      <img className="img-size" src={cdn(item.photo, item.car_id, 1)} alt='imgs'></img>
                    </Col>
                    <Col lg='8'>
                      <Row className="card-content">
                        <Col className="col-padding mv-2" lg='8' md='6' sm='6' xs='12' >
                          <p className="title title-media d-none-xs">{man_search(item.man_id, mans)?.man_name} <span className="title_span"> {item.car_model}</span></p>
                          <div className="d-none-xs">
                            <div className="flex flex-between flex-evenly">
                              <p className="small-desc">{union}ჰიბრიდი</p>
                              <p className="small-desc">{spidometri}ჰიბრიდი</p>
                            </div>
                            <div className="flex flex-between flex-evenly">
                              <p className="small-desc">{gadamrtveli} ავტომატიკა</p>
                              <p className="small-desc">{ruli} მარჯვენა</p>
                            </div>
                          </div>
                          ...
                          <p className="views views-margin-top  d-none-xs">{item.views} ნახვა・{day_different(item.order_date)} {day_different(item.order_date === 1) ? 'წუთის წინ' : 'დღის წინ'}</p>
                        </Col>
                        <Col className="flex-end-product mr-60 mv-2" lg='4' md='6' sm='6' xs='12' >
                          <p className="align-text d-none-xs">{item.customs_passed ? <span className="green-color ">{done} განბაჟებული {flag_geo}</span> : <span className="orange-color">განუბაჟებელი {<img src={flag_usa} alt="flags" />}</span>}</p>
                          <h1 className="align-text align-text-start price price_media d-none-xs">{Math.round(item.price_usd * currency.course[currency.current].price).toLocaleString('en-US', { maximumFractionDigits: 2 })}</h1>
                          <div className="flex flex-end padd-left icons-margin-top icons-margin-none d-none-xs">
                            <button className="btn-default ">{heart}</button>
                            <button className="btn-default ">{redact}</button>
                            <button className="btn-default ">{manqana}</button>
                          </div>
                          <div className="d-flex jc-between ai-center d-none-lg">
                            <p className="m-0">{item.views} ნახვა・{day_different(item.order_date)} {day_different(item.order_date === 1) ? 'წუთის წინ' : 'დღის წინ'}</p>
                            <div className="d-flex jc-between ai-center">
                              <button className="btn-default ">{heart}</button>
                              <button className="btn-default ">{redact}</button>
                              <button className="btn-default ">{manqana}</button>
